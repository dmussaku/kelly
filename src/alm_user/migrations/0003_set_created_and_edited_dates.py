...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for user in orm.User.objects.all():
            try:
                user.date_created = user.subscriptions.all().order_by("date_created").first().date_created
                user.date_edited = user.subscriptions.all().order_by("date_edited").first().date_edited
            except:
                user.date_created = datetime.datetime(2015, 4, 1)
                user.date_edited = user.date_created
            finally:
                user.save()
    ...
    complete_apps = ['almanet', 'alm_user']
    symmetrical = True
