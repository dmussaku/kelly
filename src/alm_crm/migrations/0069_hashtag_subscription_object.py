...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for htr in orm.HashTagReference.objects.all():
            old_hashtag = htr.hashtag
            new_hashtag = None
            try:
                new_hashtag = orm.HashTag.objects.get(text=old_hashtag.text, company_id=htr.company_id)
            except orm.HashTag.DoesNotExist:
                new_hashtag = orm.HashTag(text=old_hashtag.text, company_id=htr.company_id)
                new_hashtag.save()
            htr.hashtag = new_hashtag
            htr.save()

        for h in orm.HashTag.objects.filter(company_id__isnull=True):
            h.delete()
