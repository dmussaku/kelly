    ...
    def forwards(self, orm):
        ...
        with transaction.atomic():
            for c in orm.Contact.objects.all():
                sales_cycle = c.sales_cycles.filter(is_global=False, latest_activity__isnull=False).order_by('-latest_activity__date_edited').first()
                c.latest_activity = sales_cycle and sales_cycle.latest_activity or None
                c.save()

            for sc in orm.SalesCycle.objects.all():
                sc.latest_activity = sc.rel_activities.order_by('-date_edited').first()
                sc.save()
