...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for sc in orm.SalesCycle.objects.filter(is_global=False):
            # только один из них сработает
            if sc.rel_activities.count() == 0 and sc.milestone is None:
                sc.status = SalesCycle.NEW

            if sc.rel_activities.count() != 0 or (sc.milestone is not None and sc.milestone.is_system == 0):
                sc.status = SalesCycle.PENDING

            if sc.milestone is not None and sc.milestone.is_system == 1:
                sc.status = SalesCycle.SUCCESSFUL

            if sc.milestone is not None and sc.milestone.is_system == 2:
                sc.status = SalesCycle.FAILED
    ...
    models = {
        ...
        u'almastorage.swiftfile': {
            ...
            'date_created': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            'date_modified': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2015, 11, 3, 0, 0)', 'auto_now': 'True', 'blank': 'True'}),
