...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for sales_cycle in orm.SalesCycle.objects.all():
            activities_count = sales_cycle.rel_activities.all().count()
            if activities_count > 0:
                milestone = sales_cycle.rel_activities.order_by('date_edited').last().milestone
                sales_cycle.milestone = milestone
                sales_cycle.save()
    def backwards(self, orm):
        "Write your backwards methods here."
        for sales_cycle in orm.SalesCycle.objects.all():
            if sales_cycle.milestone != None:
                sales_cycle.milestone.delete()
