...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        MILESTONES = [
            (u"Подготовка к туру", '#CCB2FF'),
            (u"Оформление документов", '#FFFF99'),
            (u"Тур", '#CCEBF5')]
        for s in orm['almanet.Subscription'].objects.filter(is_active=True):
            for mile, color_code in MILESTONES:
                milestone = orm.Milestone(
                    title=mile, color_code=color_code)
                milestone.subscription_id = s.pk
                milestone.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        orm.Milestone.objects.all().delete()
