...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for subscription in orm['almanet.Subscription'].objects.all():
            success_milestone = orm.Milestone(title='Успешно завершено', 
                                                is_system=1,
                                                color_code="#9CF4A7",
                                                subscription_id=subscription.id)
            success_milestone.save()

            fail_milestone = orm.Milestone(title='Не реализовано', 
                                                is_system=2,
                                                color_code="#F4A09C",
                                                subscription_id=subscription.id)
            fail_milestone.save()
    ...
    def backwards(self, orm):
        ...
        for subscription in orm['almanet.Subscription'].objects.all():
            success_milestone = orm.Milestone.objects.get(subscription_id=subscription.id, 
                                                            is_system=1)
            success_milestone.delete()

            fail_milestone = orm.Milestone.objects.get(subscription_id=subscription.id, 
                                                            is_system=2)
            fail_milestone.delete()
